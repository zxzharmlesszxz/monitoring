...
$needed_servers = dbquery("SELECT count(*) FROM ".DB_SERVERS . " WHERE server_new != 1 AND server_status != 0 AND server_game = 'css' AND server_off != 1");
...
if(count($pg_info['pages']) > 1) {
	...
	if($pg_info['current'] == 1) {
		...
		echo "<a href='/css/{$pg_info['previous']}'>Назад</a>&nbsp;";
		echo "<a href='/css/1'>1</a>";
	}
	echo "&nbsp;";
	...
	if($pg_info['current'] == $pg_info['last']) {
		...
		echo "<a href='/css/{$pg_info['last']}'>{$pg_info['last']}</a>&nbsp;";
		echo "<a href='/css/{$pg_info['next']}'>Вперёд</a>";
