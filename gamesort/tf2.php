...
$needed_servers = dbquery("SELECT count(*) FROM ".DB_SERVERS . " WHERE server_new != 1 AND server_status != 0 AND server_game = 'tf2' AND server_off != 1");
...
$select_query = "SELECT * FROM ".DB_SERVERS . " WHERE server_new != 1 AND server_status != 0 AND server_game = 'tf2' AND server_off != 1 ORDER BY server_vip DESC, votes DESC ".$query_limit;
...
/* TABLE BODY */
 ...
 if ($servers_total !=0 ) {
  while ($r=dbarray_fetch($servers)) {
   ...
   $row .= ($r['server_vip'] == 1) ? "<img src='/images/vip.png' align='texttop' style='opacity:0.8;'>" : "<span class='votes_count' id='votes_count_{$r['server_id']}' >{$r['votes']}</span>
 ...
 if (count($pg_info['pages']) > 1) {
  ...
  if ($pg_info['current'] == 1) {
   ...
   echo "<a href='/tf2/{$pg_info['previous']}'>Назад</a>&nbsp;<a href='/tf2/1'>1</a>";
  }
  ...
  if ($pg_info['current'] == $pg_info['last']) {
   ...
   echo <<<EOT
<a href='/tf2/{$pg_info['last']}'>{$pg_info['last']}</a>
&nbsp;
<a href='/tf2/{$pg_info['next']}'>Вперёд</a>
<div style='float:left;'>
 <a href='/#top'>Вверх сайта</a>
 </div>
