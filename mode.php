...
$needed_servers = dbquery("SELECT count(*) FROM ".DB_SERVERS . " WHERE server_new != 1 AND server_status != 0 AND server_mode = '".$_GET['type']."' AND server_off != 1");
...
$select_query = "SELECT * FROM ".DB_SERVERS." WHERE server_new != 1 AND server_status != 0 AND server_mode = '".$_GET['type']."' AND server_off != 1 ORDER BY server_vip DESC, votes DESC ".$query_limit;
 ...
 */
 ...
 if (count($pg_info['pages']) > 1) {
  ...
  if ($pg_info['current'] == $pg_info['last']) {
   ...
   echo "<div style='float:left;'><a href='/#top'>Вверх сайта</a></div>";
   echo "<div style='float:right;'><a href='/#top'>Вверх сайта</a></div>";
  }
