...
if($page == 'advanced_search') {
 ...
 <div class='cont'>
<p>Просмотр страницы запрещен</p>
...
if($page == 'adv_search_results' or $page == 'quick_search') {
 ...
 echo "
  <thead>
   ...
   <th width=160><font color='#FFF' size='2'>Адрес сервера </font></th>
   <th width=240><center><font color='#FFF' size='2'>Мод </font></center></th>
    <th width=140><font color='#FFF' size='2'>Карта </font></th>
   ...
   <th width=60><center><font color='#FFF' size='2'>Рейтинг</a></font></center></th>
    <th style='padding-right:20px;'><center>Выбор </center></th>
   </tr>
  </thead>
  ";
 ...
 if(count($query_params) == 0) {
  ...
  if(db()->num_rows($servers) != 0) {
   while($r=dbarray_fetch($servers)) {
    ...
    $row = "<tr>
      <td align='left' style='padding-left:20px;'><a href='".$settings['site_url']."pay/{$r['server_id']}'><img src='images/flags/$server_location.png' style='width:16;height:11;opacity:0.7;' title='$r[server_location]' alt='$r[server_location]'></a>&nbsp;&nbsp;<a href='".$settings['site_url']."server/{$r['server_id']}'>{$r['server_name']}</a>  ".(($r['server_steam'] == '1') ? '<img src=\'images/icon_steam.png\'>' : '')."</a></td>
    <td><img src='/images/icons/$r[server_game].gif' style='width:16px;height:16px;vertical-align:middle;margin-bottom:2px;opacity:0.7;' title='$r[server_game] сервер'  alt='$r[server_game] сервер'  /></a>&nbsp;&nbsp;{$r['server_ip']} </td>
      <td><center>$r[server_mode]</center></td>
      <td>{$r['server_map']}</center></td>
      <td><center>$players</center></td>
      <td><center>{$r['votes']}</center></td>
      <td style='padding-right:20px;'><center><a href='".$settings['site_url']."pay/{$r['server_id']}'>Выбрать сервер</a></center></td>
      </tr>";
   ...
   echo "<tr><td colspan='6' width='100%' class='noservincat'>Ничего не найдено</td></tr>";
  }
 }
 /* TBODY END */
 echo "</tbody></table>";
