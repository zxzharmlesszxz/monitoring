...
$percent = 0;							
if($server['server_maxplayers'] != 0) $percent = floor($server['server_players'] / $server['server_maxplayers'] * 100);

$steam = $server['server_steam'];
if($steam == '1') {
	$steam = 'Да';
} else {
	$steam = 'Нет';
...
if($server['server_top'] == 0 or empty($server['server_top'])) {
	$top_place = 'Не находится в топе';
} else {
	$top_place = $server['server_top'];
}
					...
					<div class='box'>
						...
						<div class='content'>
							<div id='server_info_leftcol'>
								<div><center><img src='$server_map_img' style='$server_map_img_style' title='{$server['server_map']}'></center></div>
							</div>
							...
							<div class='row'>
								<label>".((!empty($site)) ? "<a href='$site' title='Перейти на сайт сервера'>" : "")."Сайт сервера".((!empty($site)) ? "</a>" : "")."</label>
								<div class='right'><input type='text' readonly value='".((!empty($site)) ? "$site" : "Нет сайта")."'></div>
							</div>
						</div>
					</div>
				</div>
				<div class='half'>
					<div class='box'>
						...
						<div class='content'>
							...
							<div class='row'>
								<label>Дата регистрации</label><div class='right'><input type='text' value='".@date("d.m.Y H:i", $server['server_regdata'])."' readonly></div>
							</div>
							<div class='row'>
...
if(mysql_num_rows($get_comments) != 0) {
	...
	echo "<div class='row'><center>Нет отзывов о данном сервере</center></div>";
}
echo "
		</div>
	</div>
