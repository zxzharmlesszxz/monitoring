...
$url = "banner.png"; // путь, название картинки
...
if (isset($_GET["serv"])) {
 ...
 //imagettftext($img, 10, 0, 40, 18, $brown, "fonts/arial.ttf", iconv('UTF-8',
 //'WINDOWS-1251', "Сервер:"));
 imagettftext($img, 10, 0, 40, 18, $brown, "fonts/arial.ttf", toUnicodeEntities(iconv('UTF-8','WINDOWS-1251',"Сервер:")));
 imagettftext($img, 10, 0, 40, 34, $white, "fonts/arialbd.ttf", isset($info['serverName']) ? $info['serverName'] : '');
 imagettftext($img, 10, 0, 320, 18, $brown, "fonts/arial.ttf", toUnicodeEntities(iconv('UTF-8','WINDOWS-1251',"Время на карте:")));
 imagettftext($img, 9, 0, 320, 34, $white, "fonts/arialbd.ttf", isset($rules['amx_timeleft']) ? $rules['amx_timeleft'] : '');
 ...
 imagettftext($img, 9, 0, 40, 108, $white, "fonts/arialbd.ttf", isset($info['mapName']) ? $info['mapName'] : '');
 imagettftext($img, 10, 0, 320, 57, $brown, "fonts/arial.ttf", toUnicodeEntities(iconv('UTF-8','WINDOWS-1251',"След.карта:")));
 imagettftext($img, 9, 0, 320, 72, $white, "fonts/arialbd.ttf", isset($rules['amx_nextmap']) ? $rules['amx_nextmap'] : '');
 ...
 imagettftext($img, 8, 0, 415, 88, $green, "fonts/arialbd.ttf", (!empty($server_full) ? $server_full : ''));
